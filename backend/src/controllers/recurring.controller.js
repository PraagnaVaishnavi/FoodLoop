import RecurringDonation from '../models/recurring.model.js';
    ...
    for (const entry of recurs) {
      if (entry.nextScheduled <= today) {
        ...
        await FoodListing.create({
          ...
          weight: entry.weight,
          expirationDate: new Date(Date.now() + entry.expirationEstimate * 3600000),
        ...
        const interval = entry.frequency === 'daily' ? 1 : entry.frequency === 'weekly' ? 7 : 30;
